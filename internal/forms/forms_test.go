...
func TestForm_MinLength(t *testing.T) {
	...
	if form.Valid() {
		t.Error("shows minlength of met when data is shorter")
	}
...
func TestForm_IsEmail(t *testing.T) {
	...
	form.IsEmail("x")
	if form.Valid() {
		t.Error("for shows valid email for non-existent field")
	}
	...
	form.IsEmail("email")
	if !form.Valid() {
		t.Error("got an invalid email when we should have it")
	}
	...
	form.IsEmail("email")
	if form.Valid() {
		t.Error("got an valid email from invalid email")
	}
}
