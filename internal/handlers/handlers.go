...
import (
	...
	"github.com/PiotrSochaczewski/GoBooking/internal/forms"
	...
	"github.com/PiotrSochaczewski/GoBooking/internal/repository/dbrepo"
...
func (m *Repository) PostReservation(w http.ResponseWriter, r *http.Request) {
	...
	err := r.ParseForm()
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't parse form - handlers l-109")
		http.Redirect(w, r, "/", http.StatusSeeOther)
		return
	}
	...
	//sent notification - to property owner
	htmlMessage = fmt.Sprintf(`
		<strong>Reservation Confirmation</strong><br>
		Room %s with number %d is confirmed: from %s to %s.
	`, reservation.Room.RoomName, reservation.RoomID, reservation.StartDate.Format("2006/01/02"), reservation.EndDate.Format("2006/01/02"))

	msg = models.MailData{
		To:      reservation.Email,
		From:    "room@reservation.com",
		Subject: "Reservation Confirmation",
		Content: htmlMessage,
	}
	m.App.MailChan <- msg
	...
	//prevent for submit the same form twice
	http.Redirect(w, r, "/reservation-summary", http.StatusSeeOther) //StatusSeeOther = 303
...
func (m *Repository) PostShowLogin(w http.ResponseWriter, r *http.Request) {
	//best practice to renewToken each time we login/logout asap
...
// AdminAllReservation show all reservation in admin tool
...
// AdminShowReservation show reservation in the admin tool
...
func (m *Repository) AdminPostShowReservation(w http.ResponseWriter, r *http.Request) {
	err := r.ParseForm()
	if err != nil {
		m.App.Session.Put(r.Context(), "error", "can't parse form - handlers l-528")
		return
	}
...
func (m *Repository) AdminProcessReservation(w http.ResponseWriter, r *http.Request) {
	...
	if year == "" {
		...
		http.Redirect(w, r, fmt.Sprintf("/admin/reservations-cakebdar?y=%s&m=%s", year, month), http.StatusSeeOther)
	}
...
func (m *Repository) AdminDeleteReservation(w http.ResponseWriter, r *http.Request) {
	...
	m.App.Session.Put(r.Context(), "flash", "Reservation sucessfully deleted")
	if year == "" {
		http.Redirect(w, r, fmt.Sprintf("/admin/reservations-%s", src), http.StatusSeeOther)
	} else {
		http.Redirect(w, r, fmt.Sprintf("/admin/reservations-cakebdar?y=%s&m=%s", year, month), http.StatusSeeOther)
...
func (m *Repository) AdminPostReservationsCalendar(w http.ResponseWriter, r *http.Request) {
	...
	for _, x := range rooms {
		//Get the block map from the session. Loop through entire map, if we have a entry in the map
		// that does not exist in our posted data, and if the restricion id > 0, then it's a block we need to remove
		curMap := m.App.Session.Get(r.Context(), fmt.Sprintf("block_map_%d", x.ID)).(map[string]int)
		for name, value := range curMap {
			// ok will be false if the value is not in the map
			if val, ok := curMap[name]; ok {
				//only pay attention to values > 0, and that are not in the form post
				// the rest are placeholder for days without blocks
				if val > 0 {
